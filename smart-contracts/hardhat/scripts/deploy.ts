...
async function main() {
  ...
  const providerMain = new ethers.JsonRpcProvider(`${process.env.DEVNET_RPC_INTERNAL}`);
  const walletMain = new ethers.Wallet(`${process.env.PRIVATE_KEY_DEVNET_INTERNAL}`, providerMain);
  ...
  console.log("\nRegistered Smart Contract - Devnet\nAddress:", addressMain)
  console.log("=================")
}
